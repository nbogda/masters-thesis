...
def make_arrays_helper(dest, set_name):
	...
	if set_name == "valid":
		set_name = "test"
	np.save("../data/%s_%s_set.npy" % (dest, set_name), imgs)
	np.save("../data/%s_%s_labels.npy" % (dest, set_name), labels)
...
def choose_filter(set, method):
	if method == "ROF":
		...
		set = filter_imgs(set, method, 5, 20, 11)
	elif method == "An Diff":
		set = filter_imgs(set, method, 0.1, 10)
	elif method == "TVD":
...
def filter_imgs_(img, method, one, two, three, four):
	...
	if method == "ROF":
		...
		filtered_img = filters.anisotropic_diffusion(i, one, two)
	elif method == "TVD":
		filtered_img = filters.TVD(i, one)
	elif method == "Resize":
		filtered_img = cv2.resize(i, (one, one), interpolation = cv2.INTER_AREA)
